...
export async function GET(request, { params }) {
  try {
    ...
    console.log(
      `[LOG PDF] Iniciando geração de PDF para Operação ID: ${operacaoId}`
    );
    ...
    if (dupError) {
      console.error(
        "[ERRO PDF] Erro ao buscar duplicatas no Supabase:",
        dupError
      );
    ...
    if (!duplicatas || duplicatas.length === 0) {
      throw new Error(
        `Nenhuma duplicata encontrada para a operação #${operacaoId}.`
      );
    }
    ...
    for (const duplicata of duplicatas) {
      if (!duplicata.operacao || !duplicata.operacao.cliente) {
        console.warn(
          `[AVISO PDF] Duplicata ${duplicata.id} sem dados de operação ou cliente. Será pulada.`
        );
        continue;
      }
      ...
      if (duplicata.sacado_id) {
        const { data: sacadoPorId } = await supabase
          .from("sacados")
          .select("*")
        ...
        // CORREÇÃO APLICADA AQUI: Busca pela matriz quando o ID não está presente.
        const { data: sacadoPorNome } = await supabase
          .from("sacados")
          .select("*")
          .eq("nome", duplicata.cliente_sacado)
          .is("matriz_id", null) // Adicione esta linha
          .single();
      ...
      if (!sacado) {
        console.warn(
          `[AVISO PDF] Sacado "${duplicata.cliente_sacado}" não encontrado para duplicata ${duplicata.id}. Será pulada.`
        );
        continue;
      }
    ...
    if (listaBoletos.length === 0) {
      throw new Error(
        "Não foi possível montar os dados para nenhum boleto da operação."
      );
    }
