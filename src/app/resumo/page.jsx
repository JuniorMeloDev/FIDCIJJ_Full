    ...
    (async () => {
      ...
      try {
        ...
        const [saldosRes, metricsRes] = await Promise.all([
          fetch(`/api/dashboard/saldos?${params}`, { headers }), // Caminho alterado
          fetch(`/api/dashboard/metrics?${params}`, { headers }), // Deixe este por enquanto, irá falhar
        ]);
              ...
              <div className="space-y-3 max-h-80 overflow-auto pr-2">
                {metrics.vencimentosProximos?.length > 0 ? (
                  ...
                  <p className="text-gray-400">
                    Nenhuma duplicata a vencer nos próximos {diasVencimento}{" "}
                    dias.
                  </p>
                )}
              </div>
