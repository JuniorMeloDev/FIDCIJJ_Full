...
  );
  ...
  const [ofxCurrentPage, setOfxCurrentPage] = useState(1);
...
  };
  ...
  const handleOfxUpload = async (file) => {
    ...
    setInterSaldo(null);
    setOfxExtrato(null);   
    setFilters(prev => ({ ...prev, contaExterna: '', contaBancaria: '' })); 

    const formData = new FormData();
    formData.append('file', file);

    try {
      showNotification('Processando arquivo OFX...', 'info');
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || 'Falha ao processar arquivo OFX.');
      }

      setOfxExtrato(data); 
      setOfxCurrentPage(1); 
      showNotification('Arquivo OFX carregado com sucesso!', 'success');
    } catch (err) {
      setError(err.message);
      showNotification(err.message, 'error');
    } finally {
...
  };
  ...
  {/* --- MODIFICAÇÃO: Recebe 'contaDestino' do modal --- */}
  const handleConfirmarConciliacao = async ({ items, detalhesTransacao, contaDestino }) => {
    try {
        let contaBancariaOrigem;

        if (filters.contaExterna) {
            // Se for API (Inter), busca o número da conta correspondente ao filtro
            const contaApi = contasMaster.find(c => c.contaBancaria.includes(filters.contaExterna));
            contaBancariaOrigem = contaApi?.contaCorrente;
        } else {
            // Se for OFX, usa a conta que o usuário selecionou no modal
            contaBancariaOrigem = contaDestino;
        }

        if (!contaBancariaOrigem) {
            throw new Error("Não foi possível identificar a conta de destino para conciliação.");
        }
...
          items,
          detalhesTransacao,
          contaBancaria: contaBancariaOrigem, // Envia a conta correta
        }),
      });
      if (!response.ok) {
...
        "success"
      );
      clearFilters(); // Limpa tudo para recarregar
    } catch (err) {
      showNotification(err.message, "error");
    }
  };
...
  }, [interExtrato, interSaldo]);
  ...
  const ofxExtratoProcessado = useMemo(() => {
    if (!ofxExtrato?.transacoes) return [];
    const groupedByDate = ofxExtrato.transacoes.reduce((acc, t) => {
      const date = t.dataEntrada;
      if (!acc[date]) acc[date] = [];
      acc[date].push(t);
      return acc;
    ...
    return sortedDates.map((date) => {
      const transactions = groupedByDate[date];
      return { date, transactions, dailyBalance: null }; 
    });
  }, [ofxExtrato]);
...
    setFilters((prev) => ({ ...prev, [e.target.name]: e.target.value }));
    if (e.target.name === 'contaExterna' && e.target.value) {
        setOfxExtrato(null);
        setOfxCurrentPage(1);
    }
  };
...
  );
  ...
  const ofxIndexOfLastItem = ofxCurrentPage * INTER_ITEMS_PER_PAGE; 
  const ofxIndexOfFirstItem = ofxIndexOfLastItem - INTER_ITEMS_PER_PAGE;
  const currentOfxItems = ofxExtratoProcessado.slice(
    ofxIndexOfFirstItem,
    ofxIndexOfLastItem
  );
...
      />
      {/* --- MODIFICAÇÃO: Passa 'saldos' e 'filters.contaExterna' para o modal --- */}
      <ConciliacaoModal
...
              <>
              {ofxExtrato ? (
                  // --- RENDERIZAÇÃO DO EXTRATO OFX ---
                  <>
                    <div className="flex-grow overflow-y-auto">
                      {currentOfxItems.length > 0 ? (
                        <div className="space-y-4">
                          {currentOfxItems.map((group) => (
                            <div key={group.date}>
                              ...
                              <ul className="divide-y divide-gray-700 bg-gray-700/50 p-2 rounded-b-md">
                                {group.transactions.map((t) => (
                                  <li
                                    key={t.idTransacao}
                                    className={`py-2 px-1 flex justify-between items-center text-sm ${
                                    ...
                                    onContextMenu={(e) => {
                                      if (t.tipoOperacao === "C" && !reconciledTransactionIds.has(t.idTransacao)) {
                                        e.preventDefault();
                                        handleConciliarTransacao(t);
                                      }
                                    }}
                                    title={t.tipoOperacao === "C" ? "Clique com o botão direito para conciliar" : ""}
                                  >
                                    <div>
                    ...
                    <div className="flex-shrink-0 pt-4">
                      <Pagination
                        totalItems={ofxExtratoProcessado.length}
                        itemsPerPage={INTER_ITEMS_PER_PAGE}
                        currentPage={ofxCurrentPage}
                        onPageChange={(page) => setOfxCurrentPage(page)}
                      />
                    </div>
                  </>
              ) : filters.contaExterna ? (
                // --- RENDERIZAÇÃO DO EXTRATO INTER (SEU CÓDIGO ORIGINAL) ---
                  <>
                    <div className="flex-grow overflow-y-auto">
...
                  </>
                ) : (
                // --- RENDERIZAÇÃO INTERNA (SEU CÓDIGO ORIGINAL) ---
                  <>
...
                              >
                image_c8eaa2.png              Valor {getSortIcon("valor")}
                              </button>
                            </th>
                          </tr>
                        </thead>
