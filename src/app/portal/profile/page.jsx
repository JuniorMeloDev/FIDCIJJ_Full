    ...
    return (
        <>
            ...
            <main className="min-h-screen pt-16 p-6 bg-gradient-to-br from-gray-900 to-gray-800 text-white">
                ...
                {user && (
                    <motion.div
                        className="bg-gray-800 p-6 rounded-lg shadow-md max-w-4xl"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ delay: 0.2 }}
                    >
                        <h2 className="text-xl font-semibold border-b border-gray-700 pb-3 mb-4">Informações da Empresa</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-400">Nome da Empresa</label>
                                <p className="mt-1 text-lg text-gray-100">{user.nome}</p>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-400">CNPJ</label>
                                <p className="mt-1 text-lg text-gray-100">{formatCnpjCpf(user.cnpj)}</p>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-400">E-mail Principal</label>
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-lg text-gray-100">{formatTelefone(user.fone) || 'Não informado'}</p>
                            </div>
                             <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-400">Endereço</label>
                                <p className="mt-1 text-lg text-gray-100">{`${user.endereco || ''}, ${user.bairro || ''} - ${user.municipio || ''}/${user.uf || ''}`}</p>
                            </div>
                             <div>
                                <label className="block text-sm font-medium text-gray-400">Usuário de Acesso</label>
