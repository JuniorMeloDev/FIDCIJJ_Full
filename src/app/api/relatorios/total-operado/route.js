...
export async function GET(request) {
    try {
        ...
        // Prepara a chamada para as funções, que já filtram por data
        const [
            valorOperadoRes, 
            topClientesRes, 
            topSacadosRes
        ...
        const errors = [valorOperadoRes.error, topClientesRes.error, topSacadosRes.error].filter(Boolean);
        if (errors.length > 0) throw new Error("Uma ou mais consultas para o relatório falharam.");
