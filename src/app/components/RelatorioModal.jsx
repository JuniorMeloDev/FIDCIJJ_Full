...
export default function RelatorioModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const fetchContas = async () => {
        try {
          const res = await fetch(`/api/dashboard/saldos`, {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const response = await fetch(
        `/api/relatorios/${endpoint}?${params.toString()}`,
        {
          headers: getAuthHeader(),
        }
      );
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            "Não foi possível buscar os dados para o relatório."
        );
      }
      const data = await response.json();

      const hasData = (type, responseData) => {
        if (type === "totalOperado")
          return (
            responseData &&
            (responseData.clientes?.length > 0 ||
              responseData.sacados?.length > 0)
          );
      ...
      // Processa e armazena os dados para a visualização ANTES de gerar o arquivo
      if (reportType === "totalOperado") {
        ...
        if (format === "pdf") {
          generatePdf(
            {
              ...data,
              abc: { cedentes: processedCedentes, sacados: processedSacados },
            },
            reportType,
            filters
          );
  ...
  const generatePdf = (data, type, currentFilters) => {
    const doc = new jsPDF({
      orientation:
        type === "fluxoCaixa" || type === "duplicatas"
          ? "landscape"
          : "portrait",
    });
    ...
    doc.setFontSize(18);
    doc.text(
      `Relatório de ${type.replace(/([A-Z])/g, " $1").trim()}`,
      pageWidth - 14,
      22,
      { align: "right" }
    );
    ...
    if (currentFilters.dataInicio || currentFilters.dataFim) {
      filterText += `Período de ${
        formatDate(currentFilters.dataInicio) || "..."
      } a ${formatDate(currentFilters.dataFim) || "..."}. `;
    }
    ...
    switch (type) {
      case "fluxoCaixa":
        ...
        body = data.map((row) => [
          formatDate(row.data_movimento),
          row.descricao,
          row.conta_bancaria,
          row.categoria,
          formatBRLNumber(row.valor),
        ]);
        ...
        let startX = 14;
        const contasParaMostrar = currentFilters.conta
          ? [currentFilters.conta]
          : contas;

        contasParaMostrar.forEach((conta) => {
          const saldo = data
            .filter((d) => d.conta_bancaria === conta)
        ...
        if (!currentFilters.conta) {
          ...
          doc.setFontSize(10);
          doc.text(
            `Total Geral do Período: ${formatBRLNumber(totalGeral)}`,
            14,
            finalY + 30
          );
        }
        ...
        autoTable(doc, {
          ...
          didDrawPage: (hookData) => {
            const totalBruto = data.reduce(
              (sum, row) => sum + (row.valor_bruto || 0),
              0
            );
            doc.setFontSize(10);
            doc.text(
              `Total Bruto das Duplicatas: ${formatBRLNumber(totalBruto)}`,
              14,
      ...
      case "totalOperado":
        doc.setFontSize(12);
        doc.text(
          `Total Operado no Período: ${formatBRLNumber(
            data.valorOperadoNoMes
          )}`,
          14,
          40
        );
        ...
        doc.text(
          `Total de Despesas: ${formatBRLNumber(data.totalDespesas)}`,
          14,
          50
        );
        doc.text(
          `Lucro Líquido: ${formatBRLNumber(data.lucroLiquido)}`,
          14,
          55
        );
        ...
        if (data.abc.cedentes.length > 0) {
          doc.addPage();
          doc.text("Análise ABC de Cedentes", 14, 20);
          autoTable(doc, {
            startY: 25,
            head: [["Classe", "Cedente", "Valor", "% Acumulado"]],
            body: data.abc.cedentes.map((c) => [
              c.classe,
              c.name,
              formatBRLNumber(c.valor),
        ...
        if (data.abc.sacados.length > 0) {
          doc.addPage();
          doc.text("Análise ABC de Sacados", 14, 20);
          autoTable(doc, {
            startY: 25,
            head: [["Classe", "Sacado", "Valor", "% Acumulado"]],
            body: data.abc.sacados.map((s) => [
              s.classe,
              s.name,
              formatBRLNumber(s.valor),
  ...
  const generateExcel = (data, type, currentFilters) => {
    ...
    switch (type) {
      case "fluxoCaixa":
        ...
        const totalGeral = data.reduce((sum, row) => sum + row.valor, 0);
        const totalsFluxo = [
          { Categoria: "Total do Período", Valor: totalGeral },
        ];
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-700 pt-4">
            <div>
              ...
              <input
                ...
                className="mt-1 w-full bg-gray-700 border-gray-600 rounded-md p-1.5 text-sm"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300">
              ...
              <>
                ...
                <div>
                  ...
                  <AutocompleteSearch
                    ...
                    onChange={handleFilterChange}
                    onSelect={(c) => handleAutocompleteSelect("cliente", c)}
                ...
                <div>
                  ...
                  <AutocompleteSearch
                    ...
                    onChange={handleFilterChange}
                    onSelect={(s) => handleAutocompleteSelect("sacado", s)}
        ...
        <div className="mt-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-orange-500 text-white font-semibold py-2 px-4 rounded-md hover:bg-orange-600 transition text-sm disabled:bg-orange-400"
            >
