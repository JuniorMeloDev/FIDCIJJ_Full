...
export async function GET(request, { params }) {
    try {
        ...
        const { data: clienteData } = await supabase
            .from('clientes')
            .select('*')
            .eq('id', operacaoData.cliente_id)
            .single();

        const { data: tipoOpData } = await supabase
            .from('tipos_operacao')
            .select('*')
            .eq('id', operacaoData.tipo_operacao_id)
            .single();

        const { data: duplicatasData } = await supabase
            .from('duplicatas')
            .select('*')
            .eq('operacao_id', id);
        ...
        if (logoBase64) {
            doc.addImage(logoBase64, 'PNG', 14, 12, 50, 18);
        }
        ...
        const head = [[
            ...
            getHeaderCell('Venc. Parcelas'),
            getHeaderCell('Sacado/Emitente'),
            getHeaderCell('Juros Parcela'),
            getHeaderCell('Valor')
        ]];
        ...
        const totaisBody = [
            ...
            ...operacao.descontos.map(d => [
                `${d.descricao}:`, { content: `-${formatBRLNumber(d.valor)}`, styles: { halign: 'right' } }
            ]),
            [
                { content: 'Líquido da Operação:', styles: { fontStyle: 'bold' } },
                { content: formatBRLNumber(operacao.valor_liquido), styles: { halign: 'right', fontStyle: 'bold' } }
            ]
        ];
        ...
        // ✅ Conversão correta do PDF para Uint8Array
        ...
        return new Response(pdfUint8Array, { headers });
