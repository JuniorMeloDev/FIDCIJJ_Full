        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-[60] p-4" onClick={onClose}>
            <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-4xl text-white flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                                ...
                                <div key={d.id} className="p-2 rounded-md bg-gray-900/50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-grow grid grid-cols-2 text-sm">
                                            <span><strong>{d.nfCte}</strong></span>
                                            <span className="text-right font-semibold">{formatBRLNumber(d.valorBruto)}</span>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2 mt-2 pl-8">
                                        <input type="text" onChange={(e) => handleItemValueChange(d.id, 'juros', e.target.value)} placeholder="Juros/Multa" className="bg-gray-700 p-1 text-xs rounded-md w-full" />
                                        <input type="text" onChange={(e) => handleItemValueChange(d.id, 'desconto', e.target.value)} placeholder="Desconto" className="bg-gray-700 p-1 text-xs rounded-md w-full" />
                                    </div>
                                </div>
                ...
                <div className="mt-4 p-3 bg-gray-900/50 rounded-md grid grid-cols-3 gap-4 text-center flex-shrink-0">
                    <div><p className="text-sm text-gray-400">Total Calculado</p><p className={`text-xl font-bold ${Math.abs(saldoRestante) > 0.01 ? 'text-red-500' : 'text-green-400'}`}>{formatBRLNumber(valorFinalCalculado)}</p></div>
                    <div><p className="text-sm text-gray-400">Valor Recebido</p><p className="text-xl font-bold text-gray-200">{formatBRLNumber(transacao.valor)}</p></div>
                    <div><p className="text-sm text-gray-400">Diferença</p><p className={`text-xl font-bold ${Math.abs(saldoRestante) > 0.01 ? 'text-red-500' : 'text-green-400'}`}>{formatBRLNumber(saldoRestante)}</p></div>
                </div>
                ...
                <div className="mt-6 flex justify-end gap-4 flex-shrink-0">
                    ...
                    <button onClick={handleConfirmar} disabled={selectedIds.size === 0} className="bg-green-600 font-semibold py-2 px-4 rounded-md hover:bg-green-700 disabled:opacity-50">Confirmar Conciliação</button>
                </div>
            </div>
        </div>
    );
